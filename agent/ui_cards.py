...
def card_hotel_rooms(context: Dict) -> Dict:
    """
    Room listing - each room with multiple images sent in sequence.
    WhatsApp doesn't support grids/sliders, so we send:
    1. First image + room details + button (main card)
    2. Additional images as separate gallery messages
    """
    ...
    for i, room in enumerate(rooms):
        ...
        caption = f"*{hotel_name}*\n"
        caption += f"*Room {i + 1}: {cat} — {rtype}*\n\n"
        caption += f"*Capacity:* ({min_cap} MINIMUM) - ({max_cap} MAXMIMUM) guests\n"
        caption += f"*Price:* Rs.{int(base):,}/night\n"
        if extra and int(extra) > 0:
            caption += f"*Extra Person:* Rs.{int(extra):,}/night\n"
        ...
        # ── MAIN ROOM CARD (First image + details + button) ──
        if room_images and len(room_images) > 0:
            ...
            if first_image and first_image.startswith(('http://', 'https://')):
                ...
                # ── ADDITIONAL IMAGES (send as separate gallery) ──
                for img_idx, img_url in enumerate(room_images[1:4], 2):  # Send next 3 images max
                    if img_url and img_url.startswith(('http://', 'https://')):
                        gallery_caption = f"📸 *{hotel_name}* — Room {i + 1}\nImage {img_idx} of {len(room_images)}"
                        responses.append({
                            "type": "image",
                            "content": img_url,
                ...
                remaining = len(room_images) - 4
                if remaining > 0:
                    responses.append({
                        "type": "text",
                        "content": f"📸 *+{remaining} more images available* for {cat} — {rtype}\nAsk 'show more images' to view them."
                    })
                
                continue
        
        # No image or invalid image URL - send as text with button
        text_content = f"*{hotel_name}*\n"
        text_content += f"*Room {i + 1}: {cat} — {rtype}*\n\n"
        caption += f"*Capacity:* ({min_cap} MINIMUM) - ({max_cap} MAXMIMUM) guests\n"
        text_content += f"*Price:* Rs.{int(base):,}/night\n"
        if extra and int(extra) > 0:
            text_content += f"*Extra Person:* Rs.{int(extra):,}/night\n"
        if facilities:
            facilities_str = ", ".join(facilities[:3])
            if len(facilities) > 3:
                facilities_str += f" +{len(facilities) - 3} more"
