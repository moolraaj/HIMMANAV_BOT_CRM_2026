...
class AIHotelAgent:
    ...
    # ─────────────────────────────────────────────────────────────
    # PACKAGE FLOW — SHOW VEHICLES DIRECTLY FROM PACKAGE DATA
    # ─────────────────────────────────────────────────────────────

    def _pkg_fetch_package_vehicles(self, context: Dict, tools: TravelTools, state: dict) -> Dict:
        """
        Read vehicles directly from the selected package's vehicles[] array.
        Package API returns full vehicle objects with:
          vehicle_name, vehicle_slug, vehicle_image, vehicle_category,
          vehicle_price, seater_capacity, seasons[]
        Normalise field names so card_vehicles_list works unchanged.
        ...
        try:
            ...
            if not raw_vehicles:
                logger.warning("Package has no vehicles[] — nothing to show")
                return {"type": "text", "content": "⚠️ No vehicles are included in this package."}

            # ── Normalise field names to match card_vehicles_list ──
            normalised = []
            for v in raw_vehicles:
                if isinstance(v, str):
                    # Old format: plain string — skip
                    logger.warning(f"Skipping plain-string vehicle entry: {v}")
                    continue
                normalised.append({
                    # Fields card_vehicles_list reads:
                    ...
                    # Keep originals for price calculation in _pkg_calculate_and_show_price:
            ...
            logger.info(f"🚗 Showing {len(normalised)} package-included vehicles (from package data)")
            from agent.ui_cards import card_vehicles_list
            return card_vehicles_list(context)

        except Exception as e:
            logger.error(f"_pkg_fetch_package_vehicles error: {e}", exc_info=True)
            return {"type": "text", "content": f"Error loading vehicles: {str(e)}"}

    # ─────────────────────────────────────────────────────────────
    # PACKAGE FLOW — FETCH PACKAGES
    # ─────────────────────────────────────────────────────────────
    ...
    def _pkg_calculate_and_show_price(self, context: Dict, tools: TravelTools, state: dict) -> Dict:
        ...
        try:
            ...
            if vehicle:
                ...
                v_base = 0
                try:
                    raw = vehicle.get("price", vehicle.get("vehicle_price", "0"))
                    v_base = float(str(raw).replace(",", ""))
                except (ValueError, TypeError):
                    v_base = 0
    ...
    def execute(self, phone: str, user_message: str, state: dict = None,
        ...
        if svc == "package":
            ...
            # ── Room category selection → go straight to packages ─
            ...
            # ── Vehicle selection → calculate price directly ───────
            ...
            # ── Package selection → show package vehicles directly ─
            if user_message.startswith("select_package_"):
                try:
                    ...
                    if idx < len(pkgs):
                        ...
                        context["vehicle"]           = None   # reset any prior vehicle
                        context["step"]              = "pkg_ask_vehicle"
                        self._save(state, context)
                        return self._pkg_fetch_package_vehicles(context, tools, state)
                except ValueError:
                    pass
        ...
        # ── Merge dates (hotel flow or package fallback) ──────────
        if svc != "package":
